...
const VulnerabilitiesBlock: React.FC<Props> = ({ vulnerabilities, lastScanDate }) => {
    return (
        <div>
            ...
            <section className={styles.vulnerabilities}>
                    ...
                    {vulnerabilityLevels.map(({ label, key, colorClass }) => {
                        ...
                        return (
                            <div key={label} className={styles.item}>
                                <div className={`${styles.count} ${colorClass}`}>{data.count}</div>
                ...
                <div className={styles.lastScan}>Последнее сканирование: {lastScanDate}</div>
            </section></div>
